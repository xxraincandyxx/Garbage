...
int _insert(Seq* a, int idx, int val) {
	if (idx < 0 || idx > a->n) { return -1; }  // index error
	if (a->cur >= a->n - 1) { return -1; }     // alloc error
...
int _remove(Seq* a, int idx) {
	if (idx < 0 || idx > a->cur) { return -1; }  // index error
...
int _find_slist(Slist* s_lst, const int& index) {
	...
	int cur = 0; Node* ptr = s_lst->head;  // Damned Warning from Nowhere
...
void _qsort_exp(std::vector<std::vector<int>>& vals) {
	...
	auto qsort = [&](auto& qsort, int lef, int rig) -> void {
		...
		int low = lef, high = rig;
		while (lef + 1 < rig) {
			while (lef < rig && vals[lef][1] < vals[rig][1]) { lef++; }
			std::swap(vals[lef], vals[rig]);
			while (lef < rig && vals[rig][1] > vals[lef][1]) { rig--; }
			std::swap(vals[lef], vals[rig]);
		}
		if (vals[lef][1] > vals[rig][1]) { std::swap(vals[lef], vals[rig]); }
		qsort(qsort, low, lef - 1);
		qsort(qsort, rig + 1, high);
	};
	qsort(qsort, 0, n-1);
...
int main(void) {
	...
	// Polynomial
	//
	std::vector<std::vector<int>> fx_vals = {
		{2, 4}, {2, 3}, {3, 1}, {6, 0}
	};
	std::vector<std::vector<int>> gx_vals = {
		{3, 3}, {1, 2}, {2, 1}, {4, 0}
	};
