...
private:
    ...
    int count_branch_leaves(btNode* node) const {
        ...
        return count_branch_leaves(node->left) + count_branch_leaves(node->right);
    }

    int count_branch_nodes(btNode* node) const {
        if (node == nullptr) { return 0; }
        return count_branch_nodes(node->left) + count_branch_nodes(node->right) + 1;
    }

    int cal_branch_depth(btNode* node) const {
        if (node == nullptr) { return 0; }
        return std::max(cal_branch_depth(node->left), cal_branch_depth(node->right)) + 1;
    }
