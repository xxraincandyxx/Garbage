...
struct hfNode {
    ...
    hfNode(char ch, int freq) : ch(ch), freq(freq), left(nullptr), right(nullptr) {}
};
...
private:
    ...
    void build_codemap(const hfNode* node, std::string str, std::unordered_map<char, std::string>& code_map) {
        if (node == nullptr) { return; }
    ...
    hfNode* build_huffmantree(const std::unordered_map<char, int>& freq_map) {
        auto comp = [](const hfNode* a, const hfNode* b) { return a->freq > b->freq; };
    ...
    void decoder(const hfNode* node, const std::string& code, std::string& text, int cur) {
        if (node->left == nullptr && node->right == nullptr) {
            text += node->ch;
        ...
        if (code[cur] == '0') { decoder(node->left, code, text, cur + 1); }
        else if (code[cur] == '1') { decoder(node->right, code, text, cur + 1); }
        return;
    }
};
